...
void system_init(void)    
{
    ...
    if(gss_E2_writed_flag != 0xbb)   //变频aa.双系统bb
    {        
        init_ram_para2();       
        init_ram_para();
        
        eeprom_write(255,0xbb);
    }
    ...
    init_freq_ctrl();          //变频控制的参数，1.有固定值                2.E2中读取的值
    db_init_debug_parm_value();
    //--------------------------------------------------
...
{   
    ...
    {
        db_first_system_run_stop_deal();     //高温压缩机控制处理程序
        
        db_secend_system_run_stop_deal();    //低温压缩机控制处理程序
    }
    //------------------------------------------------------
    //输出程序
...
void timer_op(void) 
{
    ...
    if (guc_1s_timer == 0)        //1s
    {
        ...
        db_comp_run_stop_ask_for_delaytime(); //第一台压机开关机请求
		...
		tank_mouth_heat();	  
		db_fan_stop_delaytime();
		//db_single_to_double_mode_switch();
        db_comp_system_fault_judge();
...
void operation_parm_read(void)
{
    ...
    //gss_factory_parm_value[DISP_FACTORY_SS] = ram_para[num_TE_fault_shield];  //双系统不用此参数
    ...
    //gss_factory_parm_value[DISP_FACTORY_A] 
    //gss_factory_parm_value[DISP_FACTORY_B] 
    gss_factory_parm_value[DISP_FACTORY_Pb]  =  ram_para[num_db_system_fault_masking];
}
