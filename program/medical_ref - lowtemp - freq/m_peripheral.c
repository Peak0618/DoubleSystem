...
void outfan2_ctrl_deal(void)  
{
    ...
    {
        if(bflg_db_first_comp_runing == 1)
        {
        ...
        {
            if(bflg_db_comp_fan_stop_delaytime == 0)   //压机停机1min后
            {
                bflg_outfan20_running = 0;
                bflg_outfan21_running = 0;
            }
...
void heat22_ctrl_deal(void)     
{
    ...
    {
        if (bflg_db_first_comp_runing == 1)                     //压机开机
        {
        ...
        if (bflg_heat20_space_delaytime == 0)      // peak 每隔16小时判断 =0是16小时后了 或初始上电
        {
            ...
            else
            {
                ...
                gss_heat20_wait_delaytimer = 0;
                                    ...
                                    //       ----->db_first_system_run_stop_deal(void) 中关机
    {
        if (bflg_db_first_comp_runing == 0)    //如果压缩机停机
        {
...
功能: 毛细管加热180秒后，最后 bflg_heat20_comp2_stop = 0; 停止对压缩机的强制停机，让
      其回到正常的控制，通过检测来设定是开还是关；承接 db_first_system_run_stop_deal(void) 函数 
...
void RL_output_deal(void)     
{
    ...
    {
        /*if (RL_A_COMP_OUT_PIN != bflg_db_first_comp_runing)     //定频压机1
        {
            RL_A_COMP_OUT_PIN = bflg_db_first_comp_runing;
            
            bflg_RL_output_delaytime = 1;
            guc_RL_output_delaytimer = 0;
        }
        else if (RL_B_COMP_OUT_PIN != bflg_db_secend_comp_runing) //定频压机2
        {
            RL_B_COMP_OUT_PIN = bflg_db_secend_comp_runing;
        ...
        if (RL_FAN1_OUT_PIN != bflg_outfan20_running) //风机1
        {
