...
void led_disp1_deal(void)      
{
    ...
    else if(guc_disp_mode == DISP_FACTORY) 
    {
        ...
        {   
            if((gsc_disp_parm_type == DISP_FACTORY_A)||(gsc_disp_parm_type == DISP_FACTORY_B))
            {
                guc_disp_rate = not_dot;
                if(gsc_disp_parm_type == DISP_FACTORY_A)
                {
                    led1_disp_val(gss_adg_Temp[6]);
                }
                else if(gsc_disp_parm_type == DISP_FACTORY_B)
                {
                    led1_disp_val(gss_adg_Temp[7]);
                }
            }
            else
            {
    ...
    else if(guc_disp_mode == DISP_TIME_SET)            //RTC时间设置
    {
        if(bflg_disp1_flicker == 1)  
        {
    ...
    else if(guc_disp_mode == DISP_DEBUG)               //设定调试参数模式
    {
        ...
        else if (gsc_disp_debug_mode == DISP_DEBUG_PARA)               //如果是显示参数模式
        {
            ...
            {
                if((gsc_disp1_para_mode == DB_P00)||(gsc_disp1_para_mode == DB_P01)||(gsc_disp1_para_mode == DB_P02)||
                   (gsc_disp1_para_mode == DB_P09)||(gsc_disp1_para_mode == DB_P10))
                {
                    guc_disp_rate = dot_0d1;
                    led1_disp_val(gss_db_debug_parm_value[gsc_disp1_para_mode]);
                }   
                else
                {
                    guc_disp_rate = not_dot;
...
{ 
   ...
   /* else if ((bflg_TC_2_fault == 1) && (bflg_error_code_alarm_TE_fault_mark ==0))     //peak 5 热交换器传感器故障 
    {
        bflg_error_code_delaytime = 1;
		...
		LED2 = data_E_dp;
		LED3 = data_all_off;
		LED4 = data_all_off;    //双系统的没有热交换器，将此报警去掉，但显示E.
    }*/
   ...
   else if (((bflg_TC_fault == 1)||(bflg_TC_2_fault == 1)) && (bflg_error_code_alarm_TC_fault_mark ==0))     //peak 7 冷凝传感器故障报警 
    {
        bflg_error_code_delaytime = 1;
    ...
    else if ((bflg_Tcouple_reverse == 1) && (bflg_error_code_alarm_TCP_reverse_mark ==0))   //AB系统的热电偶插反报警
    {
        bflg_error_code_delaytime = 1;
	...
	else
	{
		...
		bflg_error_code_alarm_TCP_reverse_mark =0;
	}
}
...
void view_error_code_delaytime (void)
{
    if (bflg_error_code_delaytime == 1) 
    {
        ...
        if (gss_error_code_delaytime >= 20)       // 100 * 20 = 2s
        {
            ...
            /*else if ((bflg_TC_2_fault == 1) && (bflg_error_code_alarm_TE_fault_mark ==0))             // 5 热交换器传感器故障
            {
            ...
            else if (((bflg_TC_fault == 1)||(bflg_TC_2_fault == 1)) && (bflg_error_code_alarm_TC_fault_mark ==0))             // 7 冷凝传感器故障
            {
            ...
            else if ((bflg_Tcouple_reverse == 1) && (bflg_error_code_alarm_TCP_reverse_mark == 0))   // AB的热电偶插反
            {
                bflg_error_code_alarm_TCP_reverse_mark = 1;
            }
        }
    }
}
...
{
    //低温压机运行的指示灯
    /*if (bflg_db_secend_comp_runing == 1)       // peak 低温压缩机运行标志--双系统去掉
    {
        LED8 |= 0x20;
    }
    else
